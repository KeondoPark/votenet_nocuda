...
class InvInterpolate_nocuda(Function):
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = grad_features.transpose(1, 2).contiguous()
...
class gather_nocuda(Function):
    ...
    def forward(ctx, features, idx):
        ...
        gather_points = _ext.gather_points_nocuda(features.cpu(), idx.cpu())
        return gather_points

    @staticmethod
    def backward(ctx, grad_out):
        idx, C, N = ctx.for_backwards
        grad_features = _ext.gather_points_grad_nocuda(grad_out.contiguous().cpu(), idx.cpu(), N)
        return grad_features, None

gather_op_nocuda = gather_nocuda.apply
